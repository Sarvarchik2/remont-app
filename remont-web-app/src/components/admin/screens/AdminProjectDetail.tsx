...
import { Project, MOCK_PROJECTS } from '../../../utils/mockData';
...
export const AdminProjectDetail: React.FC<AdminProjectDetailProps> = ({ projectId, lang, onBack }) => {
  const t = translations[lang].admin.project;
  const initialProject = MOCK_PROJECTS.find(p => p.id === String(projectId)) || MOCK_PROJECTS[0];
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-24 md:pb-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wide border w-fit ${
            project.status === 'process' ? 'bg-black text-white border-black' : 
                 ...
                 <div className="flex gap-3">
                    {['info', 'photo', 'doc'].map((t) => (
                      <button 
                        ...
                        className={`flex-1 py-4 rounded-2xl font-bold text-xs uppercase tracking-wider transition-all border ${
                          eventType === t 
