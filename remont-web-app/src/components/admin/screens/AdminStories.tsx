...
import { unsplash_tool } from '../../../tools/unsplash'; // Wait, I can't import tools directly. I have to simulate or implement the logic inside the component if it's UI.

// Since I cannot call the tool directly from the component code running in the browser,
// I will provide a UI to input the URL manually or a "Simulate Search" button that just sets a placeholder for now,
// OR I rely on the user pasting a URL.
// Actually, I can use a simple input for URL.
...
export const AdminStories: React.FC<AdminStoriesProps> = ({ lang, stories, onUpdateStories }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.imageUrl || !formData.title?.ru || !formData.title?.uz) {
          ...
          <div key={story.id} className="group relative aspect-[9/16] bg-slate-100 rounded-2xl overflow-hidden shadow-sm border border-slate-100">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent p-4 flex flex-col justify-end">
              <span className="text-[10px] font-bold text-white/60 uppercase tracking-wider mb-1">
                {categories.find(c => c.id === story.category)?.label.split('/')[lang === 'ru' ? 0 : 1]}
              </span>
          ...
          <div className="bg-white rounded-[32px] w-full max-w-lg p-8 shadow-2xl animate-scale-up">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-slate-900">
                {editingStory 
                  ? (lang === 'ru' ? 'Редактировать сторис' : 'Tahrirlash') 
                  : (lang === 'ru' ? 'Новая сторис' : 'Yangi story')}
              </h3>
              <button 
