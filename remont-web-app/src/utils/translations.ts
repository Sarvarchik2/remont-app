export type Language = 'ru' | 'uz' | 'en';

export const translations = {
  ru: {
    catalog: {
      title: "Каталог товаров",
      search: "Поиск товаров...",
      categories: {
        all: "Все",
        materials: "Материалы",
        furniture: "Мебель",
        lighting: "Освещение",
        plumbing: "Сантехника",
        decor: "Декор"
      },
      details: {
        specs: "Характеристики",
        desc: "Описание",
        price: "Цена"
      },
      not_found: "Ничего не найдено",
      try_again: "Попробуйте изменить параметры поиска",
      currency: "сум",
      product_not_found: "Товар не найден",
      available: "В наличии",
      learn_more: "Узнать подробнее",
      success_inquiry: "Запрос отправлен!",
      success_inquiry_desc: "Наш менеджер скоро свяжется с вами",
    },
    nav: {
      home: "Главная",
      calc: "Калькулятор",
      works: "Портфолио",
      catalog: "Каталог",
      profile: "Кабинет",
      portfolio: "Портфолио",
      dashboard: "Кабинет",
      services: "Услуги"
    },
    home: {
      welcome: "ДОБРО ПОЖАЛОВАТЬ",
      heroTitle: "Ремонт вашей мечты",
      heroInspire: "Ремонт,\nкоторый вдохновляет",
      heroComfort: "Создаем уют в каждом метре",
      cta: "Рассчитать бюджет",
      readySolutions: "ГОТОВЫЕ РЕШЕНИЯ",
      popularServices: "Популярные услуги",
      allServices: "Все услуги",
      fromPrice: "от $100",
      viewAll: "Смотреть все",
      advantages: "НАШИ ПРЕИМУЩЕСТВА",
      whyUs: "Почему мы?",
      deadlineTitle: "Сроки по договору",
      deadlineDesc: "Гарантируем выполнение в срок или штраф. Каждый день просрочки = 1% от стоимости возвращаем.",
      transparentTitle: "Прозрачный процесс",
      transparentDesc: "Ежедневные фото-отчеты в Telegram и полная прозрачность всех работ",
      materialsTitle: "Премиум материалы",
      materialsDesc: "Работаем с ведущими брендами по оптовым ценам",
      projectsCount: "Проектов",
      warranty: "Гарантия",
      warranty_years: "3 года",
      experience: "Опыт",
      experience_years: "8 лет",
      portfolioLabel: "ПОРТФОЛИО",
      all: "Все",
      stories: {
        process: "Процесс",
        reviews: "Отзывы",
        team: "Команда",
        promo: "Акции",
        details: "Подробнее",
        now: "сейчас",
        m: "м",
        h: "ч",
        d: "д"
      },
      heroSubtitle: "Премиальное качество. Сроки по договору.",
      videoTitle: "Видео-обзоры квартир",
      recentTitle: "Последние работы",
      fab: "Задать вопрос",
    },
    calc: {
      title: "Умный расчет 2.0",
      step1: "Объект и комнаты",
      step2: "Площадь квартиры",
      step3: "Состояние сейчас",
      step4: "Выберите пакет",
      types: {
        new: "Новостройка",
        secondary: "Вторичка",
        house: "Дом",
      },
      rooms: {
        studio: "Студия",
        1: "1 к",
        2: "2 к",
        3: "3 к",
        4: "4+",
      },
      conditions: {
        rough: "Черновая коробка",
        old: "Старый ремонт (Демонтаж)",
        white: "White Box (Предчистовая)",
      },
      packages: {
        economy: "Эконом",
        economy_desc: "Обои, линолеум, натяжной потолок",
        comfort: "Комфорт",
        comfort_desc: "Покраска, ламинат, гипсокартон",
        premium: "Премиум",
        premium_desc: "Декор. штукатурка, паркет, сложные потолки",
      },
      result: {
        title: "Предварительная смета",
        work: "Стоимость ��абот",
        rough_mat: "Черновые материалы",
        finish_mat: "Чистовые материалы",
        total: "Итого под ключ:",
        range: "$",
        download: "Скачать смету (PDF)",
        cta: "Вызвать инженера",
      },
      next: "Далее",
      back: "Назад",
      subtitle: "КАЛЬКУЛЯТОР",
      est_cost: "Примерная стоимость",
      area_label: "Площадь квартиры",
      sqm: "м²",
      type_label: "Тип помещения",
      level_label: "Уровень ремонта",
      cta_main: "Заказать расчет",
      disclaimer: "Расчет является предварительным. Точная стоимость определяется после замера и составления детальной сметы.",
      modal: {
        title: "Оставьте контакты",
        name: "Ваше имя",
        name_placeholder: "Введите имя",
        phone: "Телефон",
        phone_placeholder: "+998 90 354 76 66",
        summary: "Расчет",
        area: "Площадь",
        type: "Тип",
        level: "Уровень",
        total: "Примерная стоимость",
        submit: "Отправить заявку",
        manager_note: "Менеджер свяжется с вами в течение 15 минут",
        success_title: "Заявка отправлена!",
        success_text: "Наш менеджер свяжется с вами в ближайшее время для уточнения деталей",
        success_btn: "Отлично"
      }
    },
    services: {
      price_list: "ПРАЙС-ЛИСТ",
      catalog_title: "Каталог работ",
      catalog_desc: "Прозрачное ценообразование на все виды ремонтных работ",
      electrical: "Электрика",
      finishing: "Отделка",
      tiles: "Плитка",
      plumbing: "Сантехника",
      full_list: "Полный прайс-лист",
      need_consultation: "Нужна консультация?",
      consultation_desc: "Поможем рассчитать точную стоимость ремонта",
      cta: "Рассчитать бюджет",
    },
    portfolio: {
      title: "Галерея проектов",
      filters: {
        all: "Все",
        kitchen: "Кухни",
        living: "Гостиные",
        bath: "Санузлы",
        bedroom: "Спальни",
        full: "Фулл проект",
      },
      cta: "Хочу так же",
      location: "Локация",
      area: "Площадь",
      term: "Срок",
      portfolio_upper: "ПОРТФОЛИО",
      all_projects: "Все проекты",
      new_buildings: "Новостройки",
      turnkey_renovation: "Полный ремонт под ключ",
      project_not_found: "Проект не найден",
    },
    project: {
      share: "Поделиться",
      term_badge: "Срок: 3.5 месяца",
      location: "Локация",
      area: "Площадь",
      cost: "Стоимость работ",
      term: "Срок реализации",
      what_was_done: "Что было сделано",
      read_more: "Читать полностью",
      hide: "Свернуть",
      materials: "Используемые материалы",
      cta: "Хочу такой же ремонт",
      video_review: "Видео-обзор объекта",
      team_title: "Команда проекта",
      works_title: "Выполненные работы",
      about_title: "О проекте",
      stats: {
        budget: "Бюджет",
        term: "Срок",
        area: "Площадь",
        type: "Тип"
      }
    },
    dashboard: {
      title: "Мой объект",
      profile: "Профиль",
      greeting: "Здравствуйте, Алишер!",
      address: "ул. Нукусская, д. 15, кв. 42",
      manager: "Ваш менеджер",
      call: "Позвонить",
      contract: "Договор №",
      id: "Ваш ID:",
      loading: "Загрузка...",
      project: "Проект",
      events: "Событий",
      admin_panel: "Админ-панель",
      admin_desc: "Управление проектами",
      my_project_title: "Мой проект",
      status: {
        process: "В работе",
        finished: "Завершен",
        new: "Новый"
      },
      stage: "Этап:",
      paid: "Оплачено:",
      payment_progress: "Прогресс оплаты",
      no_projects: "Нет активных проектов",
      no_projects_desc: "Начните свой идеальный ремонт вместе с нами. Рассчитайте стоимость за 1 минуту.",
      calc_btn: "Рассчитать стоимость ремонта",
      contact_manager: "Связаться с менеджером",
      finance: {
        title: "Финансы",
        total: "Общая смета",
        paid: "Оплачено",
        left: "Остаток к оплате",
        history: "История платежей",
        transactions: "операций",
      },
      stage_info: {
        title: "Текущий статус",
        name: "Этап 3: Чистовая отделка",
        status: "В работе",
        forecast: "Прогноз завершения:",
        updates: "обновлений",
      },
      timeline: {
        title: "Хронология работ",
        today: "Сегодня",
        yesterday: "Вчера",
        comment1: "Закончили фартук на кухне, завтра затирка",
        comment2: "Доставка материалов (клей, затирка)",
      },
      docs: {
        title: "Документы",
        contract: "Договор №145",
        estimate: "Смета (Версия 2)",
        act: "Акт приемки черн. работ",
      },
      auth: {
        placeholder: "Номер договора (напр. 1234)",
        btn: "Войти в кабинет",
      },
      video: {
        btn: "Смотреть трансляцию 24/7",
        offline: "Камера временно недоступна",
      },
    },
    booking: {
      title: "Запись на замер",
      share_contact: "Поделиться контактом Telegram",
      or: "Или введите вручную",
      name: "Ваше имя",
      phone: "Номер телефона",
      date: "Удобное время",
      address: "Адрес объекта",
      comment: "Пожелания",
      submit: "Подтве��дить встречу",
      success: {
        title: "Заявка принята!",
        text: "Инженер свяжется с вами для подтверждения времени.",
        home: "На главную",
      },
    },
    admin: {
      nav: {
        dashboard: "Дашборд",
        crm: "Заявки (CRM)",
        users: "Пользователи",
        projects: "Проекты",
        portfolio: "Портфолио",
        services: "Услуги",
        stories: "Сторис",
        catalog: "Магазин",
        settings: "Настройки",
        logout: "Выйти",
      },
      dashboard: {
        new_leads: "Новых лидов",
        active_projects: "Активных проектов",
        total_money: "Общий оборот",
        recent_activity: "Последние действия",
        completed_projects: "Завершено проектов",
        total_clients: "Всего клиентов",
        waiting_measuring: "Ожидают замер",
        average_rating: "Средний рейтинг",
        upcoming_tasks: "Предстоящие задачи",
        overview: "Обзор",
        in_progress_now: "в работе прямо сейчас",
        require_processing: "требуют обработки",
        based_on_projects: "сумма всех платежей",
        about: "Примерно",
        total: "Всего",
        recent_time: "Недавно",
        system: "Система",
        new_lead: "Новый лид:",
        unknown: "Без имени",
        updated: "Обновлено",
        project: "Проект",
        measuring: "Замер:",
        call: "Позвонить",
        periods: {
          today: 'Сегодня',
          week: 'Эта неделя',
          month: 'Этот месяц',
          all: 'За все время'
        },
        filter: "Фильтр",
        all_tasks: "Все задачи",
        history: "История"
      },
      crm: {
        title: "Управление заявками",
        status: {
          new: "Новая",
          process: "В работе",
          measure: "Замер",
          contract: "Договор",
          reject: "Отказ",
          contacted: "Связались",
          all: "Все"
        },
        sources: {
          calculator: "Калькулятор",
          booking: "Запись на замер",
          phone: "Телефон",
          other: "Другое"
        },
        stats: {
          total: "Всего заявок",
          new: "Новых",
          in_progress: "В работе",
          contracts: "Договоров"
        },
        search_placeholder: "Поиск по имени или телефону...",
        no_leads_title: "Заявок пока нет",
        no_leads_desc: "Новые заявки появятся здесь, когда клиенты заполнят форму",
        no_name: "Имя не указано",
        fields: {
          area: "Площадь",
          type: "Тип",
          budget: "Бюджет"
        },
        actions: {
          call: "Позвонить",
          accept: "Принять"
        },
        convert: "Создать проект",
      },
      project: {
        title: "Управление проектом",
        finance_title: "Финансы",
        total_estimate: "Общая смета",
        paid_amount: "Оплачено",
        timeline_title: "Лента событий",
        add_event: "Добавить событие",
        event_placeholder: "Что сделано? (напр. Завезли ламинат)",
        upload_photo: "Загрузить фото",
        upload_doc: "Загрузить документ (PDF)",
        add_project: "Добавить проект",
        search_placeholder: "Поиск по названию ЖК, клиенту или адресу...",
        status: {
          in_progress: "В работе",
          finished: "Завершен",
          new: "Новый"
        },
        payment: "Оплата",
        paid: "Оплачено",
        new_project: "Новый проект",
        create_desc: "Создайте карточку нового объекта для отслеживания",
        input_lang: "Язык ввода",
        client: "Клиент",
        select_from_db: "Выбрать из базы",
        search_client: "Поиск клиента...",
        or_manual: "Или вручную",
        fio: "ФИО Клиента",
        phone: "Телефон",
        object: "Объект",
        address: "Адрес",
        contract_details: "Номер договора / Смета",
        total: "Всего",
        tg_id: "Telegram ID",
        terms: "Сроки",
        start_date: "Дата начала",
        deadline: "Дедлайн (Прогноз)",
        cancel: "Отмена",
        create: "Создать проект",
        foreman_salary: "ЗП Прораба",
        monthly_rate: "Ставка в мес.",
        add_month: "Добавить месяц",
        paid_label: "Оплачено",
        not_paid_label: "Ожидает",
        video_review: "Видео обзор",
        upload_video: "Загрузить видео",
        video_url: "Ссылка на видео",
        works_completed: "Выполненные работы",
        team: "Команда проекта",
        budget: "Бюджет",
        currency: "$",
        edit_project: "Редактировать проект",
        general_tab: "Основное",
        details_tab: "Детали",
        media_tab: "Медиа",
        info: "Информация",
        project_name: "Название ЖК / Проекта",
        room_type: "Тип помещения",
        area_sqm: "Площадь (м²)",
        term_realization: "Срок реализации",
        details: "Подробности",
        description: "Описание",
        tags: "Теги (через запятую)",
        materials_input: "Материалы (через запятую)",
        new_building: "Новостройка",
        new_building_desc: "Отметьте, если это новый жилой комплекс",
        works_placeholder: "Черновая: Стяжка, Штукатурка\nЧистовая: Покраска, Обои",
        team_placeholder: "Иван Иванов | Дизайнер\nПетр Петров | Прораб",
        photo_before: "Фото ДО",
        photo_after: "Фото ПОСЛЕ",
        gallery: "Галерея проекта (Все фото)",
        save_changes: "Сохранить изменения",
        publish: "Опубликовать проект",
        location: "Локация",
      },
      catalog: {
        title: "Каталог товаров",
        add_product: "Добавить товар",
        search: "Поиск по названию...",
        edit: "Редактировать товар",
        new: "Новый товар",
        main_info: "Основная информация",
        specs: "Характеристики",
        main_photo: "Главное фото",
        gallery: "Галерея",
        video: "Видео (необязательно)",
        upload_video: "Загрузить видео файл",
        video_url: "Ссылка на видео",
        save: "Сохранить",
        add: "Добавить",
        cancel: "Отмена",
        confirm_delete: "Вы уверены, что хотите удалить этот товар?",
        input_lang: "Язык ввода",
        price_label: "Цена",
        media_assets: "Медиа-ресурсы",
        title_label: "Название",
        desc_label: "Описание",
        category_label: "Категория",
        price_uzs: "Цена ($)"
      },
      portfolio: {
        title: "Наши работы",
        add_project: "Добавить проект"
      },
      stories: {
        title: "Сторис",
        add_story: "Добавить сторис",
        edit_story: "Редактировать сторис",
        new_story: "Новая сторис",
        titles: "Заголовки",
        category: "Категория",
        cover: "Обложка",
        video_optional: "Видео (необязательно)",
        video_review: "Видео обзор",
        upload_video: "Загрузить видео",
        video_url: "Ссылка на видео",
        save: "Сохранить",
        add: "Добавить",
        confirm_delete: "Вы уверены?",
        required_fields: "Заполните все обязательные поля"
      },
      users: {
        title: "Пользователи бота",
        search: "Поиск по имени, username или Telegram ID...",
        table: {
          user: "Пользователь",
          tg_id: "Telegram ID",
          status: "Статус",
          actions: "Действия"
        },
        status: {
          active: "Активен",
          inactive: "Неактивен"
        },
        not_found: "Пользователей не найдено"
      },
      settings: {
        title: "Цены и Калькулятор",
        base_price: "Базовая цена за м²",
      }
    },
  },
  uz: {
    catalog: {
      title: "Mahsulotlar katalogi",
      search: "Mahsulotlarni qidirish...",
      categories: {
        all: "Barchasi",
        materials: "Materiallar",
        furniture: "Mebel",
        lighting: "Yoritish",
        plumbing: "Santexnika",
        decor: "Dekor"
      },
      details: {
        specs: "Xususiyatlar",
        desc: "Tavsif",
        price: "Narx"
      },
      not_found: "Hech narsa topilmadi",
      try_again: "Qidiruv parametrlarini o'zgartirib ko'ring",
      currency: "so'm",
      product_not_found: "Mahsulot topilmadi",
      available: "Mavjud",
      learn_more: "Batafsil ma'lumot",
      success_inquiry: "So'rov yuborildi!",
      success_inquiry_desc: "Menejerimiz tez orada bog'lanadi",
    },
    nav: {
      home: "Asosiy",
      calc: "Hisoblash",
      works: "Ishlar",
      catalog: "Katalog",
      profile: "Kabinet",
      portfolio: "Portfolio",
      dashboard: "Kabinet",
      services: "Xizmatlar"
    },
    home: {
      welcome: "XUSH KELIBSIZ",
      heroTitle: "Sizning orzuingizdagi ta'mir",
      heroInspire: "Ilhomlantiruvchi\nta'mirlash",
      heroComfort: "Har bir metrda qulaylik yaratamiz",
      cta: "Budjetni hisoblash",
      readySolutions: "TAYYOR YECHIMLAR",
      popularServices: "Ommabop xizmatlar",
      allServices: "Barcha xizmatlar",
      fromPrice: "$100 dan",
      viewAll: "Barchasini ko'rish",
      advantages: "BIZNING AFZALLIKLARIMIZ",
      whyUs: "Nima uchun biz?",
      deadlineTitle: "Shartnoma muddatlari",
      deadlineDesc: "Muddati kafolatlanadi yoki jarima. Har bir kechikkan kun uchun = 1% qaytariladi.",
      transparentTitle: "Shaffof jarayon",
      transparentDesc: "Telegramda kundalik foto-hisobotlar va barcha ishlar jarayonining to'liq nazorati",
      materialsTitle: "Premium materiallar",
      materialsDesc: "Yetakchi brendlar bilan ulgurji narxlarda ishlaymiz",
      projectsCount: "Loyihalar",
      warranty: "Kafolat",
      warranty_years: "3 yil",
      experience: "Tajriba",
      experience_years: "8 yil",
      portfolioLabel: "PORTFOLIO",
      all: "Barchasi",
      stories: {
        process: "Jarayon",
        reviews: "Sharhlar",
        team: "Jamoa",
        promo: "Aksiyalar",
        details: "Batafsil",
        now: "hozir",
        m: "daq",
        h: "s",
        d: "k"
      },
      heroSubtitle: "Premium sifat. Shartnoma bo'yicha muddat.",
      videoTitle: "Video sharhlar",
      recentTitle: "So'nggi ishlar",
      fab: "Savol berish",
    },
    calc: {
      title: "Aqlli hisoblagich 2.0",
      step1: "Obyekt va xonalar",
      step2: "Kvartira maydoni",
      step3: "Hozirgi holat",
      step4: "Paketni tanlang",
      types: {
        new: "Yangi bino",
        secondary: "Ikkilamchi",
        house: "Uy",
      },
      rooms: {
        studio: "Studio",
        1: "1 xona",
        2: "2 xona",
        3: "3 xona",
        4: "4+",
      },
      conditions: {
        rough: "Qora suvoq",
        old: "Eski ta'mir (Demontaj)",
        white: "Oq suvoq",
      },
      packages: {
        economy: "Ekonom",
        economy_desc: "Gulqog'oz, linoleum, tortma shift",
        comfort: "Komfort",
        comfort_desc: "Bo'yoq, laminat, gipsokarton",
        premium: "Premium",
        premium_desc: "Dekor. suvoq, parket, murakkab shiftlar",
      },
      result: {
        title: "Dastlabki smeta",
        work: "Ish narxi",
        rough_mat: "Qora materiallar",
        finish_mat: "Toza materiallar",
        total: "Jami kalitgacha:",
        range: "$",
        download: "Smetani yuklab olish (PDF)",
        cta: "Muhandisni chaqirish",
      },
      next: "Keyingisi",
      back: "Orqaga",
      subtitle: "KALKULYATOR",
      est_cost: "Taxminiy qiymat",
      area_label: "Kvartira maydoni",
      sqm: "m²",
      type_label: "Xona turi",
      level_label: "Ta'mirlash darajasi",
      cta_main: "Hisobni buyurtma qilish",
      disclaimer: "Hisob-kitob dastlabki hisoblanadi. Aniq qiymat o'lchov va batafsil smeta tuzilganidan keyin aniqlanadi.",
      modal: {
        title: "Kontakt qoldiring",
        name: "Ismingiz",
        name_placeholder: "Ismni kiriting",
        phone: "Telefon",
        phone_placeholder: "+998 90 354 76 66",
        summary: "Hisob-kitob",
        area: "Maydon",
        type: "Tur",
        level: "Daraja",
        total: "Taxminiy qiymat",
        submit: "Ariza yuborish",
        manager_note: "Menejer 15 daqiqa ichida siz bilan bog'lanadi",
        success_title: "Ariza yuborildi!",
        success_text: "Menejerimiz tez orada batafsil ma'lumot olish uchun bog'lanadi",
        success_btn: "Zo'r"
      }
    },
    services: {
      price_list: "PRICEY RO'YXAT",
      catalog_title: "Ishlar katalogi",
      catalog_desc: "Barcha turdagi ta'mirlash ishlari uchun shaffof narxlar",
      electrical: "Elektr",
      finishing: "Pardozlash",
      tiles: "Kafel",
      plumbing: "Santexnika",
      full_list: "To'liq narxlar",
      need_consultation: "Maslahat kerakmi?",
      consultation_desc: "Ta'mirlashning aniq narxini hisoblashga yordam beramiz",
      cta: "Budjetni hisoblash",
    },
    portfolio: {
      title: "Loyihalar galereyasi",
      filters: {
        all: "Barchasi",
        kitchen: "Oshxona",
        living: "Mehmonxona",
        bath: "Hammom",
        bedroom: "Yotoqxona",
        full: "To'liq loyiha",
      },
      cta: "Men ham shunday xohlayman",
      location: "Manzil",
      area: "Maydon",
      term: "Muddat",
      portfolio_upper: "PORTFOLIO",
      all_projects: "Barcha loyihalar",
      new_buildings: "Yangi binolar",
      turnkey_renovation: "To'liq kalit taslim ta'mirlash",
      project_not_found: "Loyiha topilmadi",
    },
    project: {
      share: "Ulashish",
      term_badge: "Muddat: 3.5 oy",
      location: "Manzil",
      area: "Maydon",
      cost: "Ish narxi",
      term: "Amalga oshirish muddati",
      what_was_done: "Nimalar qilindi",
      read_more: "To'liq o'qish",
      hide: "Yashirish",
      materials: "Ishlatilgan materiallar",
      cta: "Xuddi shunday ta'mirni xohlayman",
      video_review: "Obyektning video sharhi",
      team_title: "Loyiha jamoasi",
      works_title: "Bajarilgan ishlar",
      about_title: "Loyiha haqida",
      stats: {
        budget: "Byudjet",
        term: "Muddat",
        area: "Maydon",
        type: "Turi"
      }
    },
    dashboard: {
      title: "Mening obyektim",
      profile: "Profil",
      greeting: "Assalomu alaykum, Alisher!",
      address: "Nukus ko'chasi, 15-uy, 42-xonadon",
      manager: "Sizning menejeringiz",
      call: "Qo'ng'iroq qilish",
      contract: "Shartnoma №",
      id: "Sizning ID:",
      loading: "Yuklanmoqda...",
      project: "Loyiha",
      events: "Hodisalar",
      admin_panel: "Admin-panel",
      admin_desc: "Loyihalarni boshqarish",
      my_project_title: "Mening loyiham",
      status: {
        process: "Jarayonda",
        finished: "Tugallangan",
        new: "Yangi"
      },
      stage: "Bosqich:",
      paid: "To'langan:",
      payment_progress: "To'lov jarayoni",
      no_projects: "Faol loyihalar yo'q",
      no_projects_desc: "Mukammal ta'miringizni biz bilan boshlang. 1 daqiqada narxni hisoblang.",
      calc_btn: "Ta'mirlash narxini hisoblash",
      contact_manager: "Menejer bilan aloqa",
      finance: {
        title: "Moliya",
        total: "Umumiy smeta",
        paid: "To'landi",
        left: "To'lov uchun qoldiq",
        history: "To'lovlar tarixi",
        transactions: "operatsiyalar",
      },
      stage_info: {
        title: "Joriy holat",
        name: "3-bosqich: Toza pardozlash",
        status: "Jarayonda",
        forecast: "Yakunlash prognozi:",
        updates: "yangilanishlar",
      },
      timeline: {
        title: "Ishlar xronologiyasi",
        today: "Bugun",
        yesterday: "Kecha",
        comment1: "Oshxonadagi plitka ishlari tugatildi",
        comment2: "Materiallar yetkazib berildi",
      },
      docs: {
        title: "Hujjatlar",
        contract: "Shartnoma №145",
        estimate: "Smeta (Versiya 2)",
        act: "Qora ishlar qabul qilish akti",
      },
      auth: {
        placeholder: "Shartnoma raqami (masalan 1234)",
        btn: "Kabinetga kirish",
      },
      video: {
        btn: "24/7 translyatsiyani ko'rish",
        offline: "Kamera vaqtincha ishlamayapti",
      },
    },
    booking: {
      title: "O'lchovga yozilish",
      share_contact: "Telegram kontaktni ulashish",
      or: "Yoki qo'lda kiriting",
      name: "Ismingiz",
      name_placeholder: "Alisher",
      phone: "Telefon raqami",
      date: "Qulay vaqt",
      address: "Obyekt manzili",
      comment: "Istaklar",
      submit: "Uchrashuvni tasdiqlash",
      success: {
        title: "Ariza qabul qilindi!",
        text: "Muhandis vaqtni tasdiqlash uchun siz bilan bog'lanadi.",
        home: "Asosiy sahifaga",
      },
    },
    admin: {
      nav: {
        dashboard: "Dashbord",
        crm: "Arizalar (CRM)",
        users: "Foydalanuvchilar",
        projects: "Loyihalar",
        portfolio: "Portfolio",
        services: "Xizmatlar",
        stories: "Stories",
        catalog: "Magazin",
        settings: "Sozlamalar",
        logout: "Chiqish",
      },
      dashboard: {
        new_leads: "Yangi arizalar",
        active_projects: "Ishdagi loyihalar",
        total_money: "Umumiy oborot",
        recent_activity: "So'nggi harakatlar",
        completed_projects: "Tugallangan loyihalar",
        total_clients: "Jami mijozlar",
        waiting_measuring: "O'lchov kutayotganlar",
        average_rating: "O'rtacha reyting",
        upcoming_tasks: "Kelgusi vazifalar",
        overview: "Umumiy ko'rinish",
        in_progress_now: "hozir ishlanyapti",
        require_processing: "ko'rib chiqish kerak",
        based_on_projects: "barcha to'lovlar yig'indisi",
        about: "Taxminan",
        total: "Jami",
        recent_time: "Yaqinda",
        system: "Tizim",
        new_lead: "Yangi ariza:",
        unknown: "Ismsiz",
        updated: "Yangilandi",
        project: "Loyiha",
        measuring: "O'lchov:",
        call: "Qo'ng'iroq",
        periods: {
          today: 'Bugun',
          week: 'Bu hafta',
          month: 'Bu oy',
          all: 'Barcha vaqt'
        },
        filter: "Filtr",
        all_tasks: "Barcha vazifalar",
        history: "Tarix"
      },
      crm: {
        title: "Arizalarni boshqarish",
        status: {
          new: "Yangi",
          process: "Jarayonda",
          measure: "O'lchov",
          contract: "Shartnoma",
          reject: "Rad etish",
          contacted: "Bog'lanildi",
          all: "Barchasi"
        },
        sources: {
          calculator: "Kalkulyator",
          booking: "O'lchovga yozilish",
          phone: "Telefon",
          other: "Boshqa"
        },
        stats: {
          total: "Jami arizalar",
          new: "Yangi",
          in_progress: "Jarayonda",
          contracts: "Shartnomalar"
        },
        search_placeholder: "Ism yoki telefon orqali qidirish...",
        no_leads_title: "Hozircha arizalar yo'q",
        no_leads_desc: "Mijozlar formani to'ldirganda yangi arizalar shu yerda paydo bo'ladi",
        no_name: "Ism ko'rsatilmagan",
        fields: {
          area: "Maydon",
          type: "Tur",
          budget: "Byudjet"
        },
        actions: {
          call: "Qo'ng'iroq",
          accept: "Qabul qilish"
        },
        convert: "Loyiha yaratish",
      },
      project: {
        title: "Loyihani boshqarish",
        finance_title: "Moliya",
        total_estimate: "Umumiy smeta",
        paid_amount: "To'landi",
        timeline_title: "Voqealar lentasi",
        add_event: "Voqea qo'shish",
        event_placeholder: "Nima qilindi? (masalan, Laminat olib kelindi)",
        upload_photo: "Rasm yuklash",
        upload_doc: "Hujjat yuklash (PDF)",
        add_project: "Loyiha qo'shish",
        search_placeholder: "Majmua, mijoz yoki manzil bo'yicha qidirish...",
        status: {
          in_progress: "Jarayonda",
          finished: "Tugallangan",
          new: "Yangi"
        },
        payment: "To'lov",
        paid: "To'landi:",
        new_project: "Yangi loyiha",
        create_desc: "Kuzatish uchun yangi obyekt kartochkasini yarating",
        input_lang: "Kiritish tili",
        client: "Mijoz",
        select_from_db: "Bazadan tanlash",
        search_client: "Mijozni qidirish...",
        or_manual: "Yoki qo'lda",
        fio: "F.I.Sh",
        phone: "Telefon",
        object: "Obyekt",
        address: "Manzil",
        contract_details: "Shartnoma raqami / Smeta",
        total: "Jami",
        tg_id: "Telegram ID",
        terms: "Muddatlar",
        start_date: "Boshlanish sanasi",
        deadline: "Dedlayn (Prognoz)",
        cancel: "Bekor qilish",
        create: "Loyiha yaratish",
        foreman_salary: "Prorab oyligi",
        monthly_rate: "Oylik stavka",
        add_month: "Oy qo'shish",
        paid_label: "To'langan",
        not_paid_label: "Kutilmoqda",
        video_review: "Video sharh",
        upload_video: "Video yuklash",
        video_url: "Video havolasi",
        works_completed: "Bajarilgan ishlar",
        team: "Loyiha jamoasi",
        budget: "Byudjet",
        currency: "$",
        edit_project: "Loyihani tahrirlash",
        general_tab: "Asosiy",
        details_tab: "Tafsilotlar",
        media_tab: "Media",
        info: "Ma'lumot",
        project_name: "TJM / Loyiha nomi",
        room_type: "Xona turi",
        area_sqm: "Maydon (m²)",
        term_realization: "Amalga oshirish muddati",
        details: "Batafsil",
        description: "Tavsif",
        tags: "Teglar (vergul bilan)",
        materials_input: "Materiallar (vergul bilan)",
        new_building: "Yangi bino",
        new_building_desc: "Agar bu yangi turar-joy majmuasi bo'lsa belgilang",
        works_placeholder: "Qora: Suvoq, Styajka\nToza: Bo'yoq, Gulqog'oz",
        team_placeholder: "Ivan Ivanov | Dizayner\nPetr Petrov | Prorab",
        photo_before: "Oldingi holat",
        photo_after: "Keyingi holat",
        gallery: "Loyiha galereyasi (Barcha rasmlar)",
        save_changes: "O'zgarishlarni saqlash",
        publish: "Loyihani chop etish",
        location: "Manzil",
      },
      catalog: {
        title: "Mahsulotlar katalogi",
        add_product: "Mahsulot qo'shish",
        search: "Nomi bo'yicha qidirish...",
        edit: "Mahsulotni tahrirlash",
        new: "Yangi mahsulot",
        main_info: "Asosiy ma'lumotlar",
        specs: "Xususiyatlari",
        main_photo: "Asosiy rasm",
        gallery: "Galereya",
        video: "Video (ixtiyoriy)",
        upload_video: "Video faylni yuklash",
        video_url: "Video havolasi",
        save: "Saqlash",
        add: "Qo'shish",
        cancel: "Bekor qilish",
        confirm_delete: "Ushbu mahsulotni o'chirib tashlamoqchimisiz?",
        input_lang: "Kiritish tili",
        price_label: "Narxi",
        media_assets: "Media resurslari",
        title_label: "Nomi",
        desc_label: "Tavsif",
        category_label: "Kategoriya",
        price_uzs: "Narxi ($)"
      },
      portfolio: {
        title: "Bizning ishlarimiz",
        add_project: "Loyiha qo'shish"
      },
      stories: {
        title: "Stories",
        add_story: "Stories qo'shish",
        edit_story: "Storiesni tahrirlash",
        new_story: "Yangi stories",
        titles: "Sarlavhalar",
        category: "Kategoriyasi",
        cover: "Muqova",
        video_optional: "Video (ixtiyoriy)",
        video_review: "Video sharh",
        upload_video: "Videoni yuklash",
        video_url: "Video havolasi",
        save: "Saqlash",
        add: "Qo'shish",
        confirm_delete: "Ishonchingiz komilmi?",
        required_fields: "Barcha majburiy maydonlarni to'ldiring"
      },
      users: {
        title: "Bot foydalanuvchilari",
        search: "Ism, username yoki Telegram ID orqali qidirish...",
        table: {
          user: "Foydalanuvchi",
          tg_id: "Telegram ID",
          status: "Holati",
          actions: "Harakatlar"
        },
        status: {
          active: "Faol",
          inactive: "Nofaol"
        },
        not_found: "Foydalanuvchilar topilmadi"
      },
      settings: {
        title: "Narxlar va Kalkulyator",
        base_price: "m² uchun bazaviy narx",
      }
    },
  },
  en: {
    catalog: {
      title: "Product Catalog",
      search: "Search products...",
      categories: {
        all: "All",
        materials: "Materials",
        furniture: "Furniture",
        lighting: "Lighting",
        plumbing: "Plumbing",
        decor: "Decor"
      },
      details: {
        specs: "Specifications",
        desc: "Description",
        price: "Price"
      },
      not_found: "Nothing found",
      try_again: "Try changing your search parameters",
      currency: "sum",
      product_not_found: "Product not found",
      available: "Available",
      learn_more: "Learn More",
      success_inquiry: "Inquiry sent!",
      success_inquiry_desc: "Our manager will contact you soon",
    },
    nav: {
      home: "Home",
      calc: "Calculator",
      works: "Works",
      catalog: "Catalog",
      profile: "Profile",
      portfolio: "Portfolio",
      dashboard: "Dashboard",
      services: "Services"
    },
    home: {
      welcome: "WELCOME",
      heroTitle: "The renovation of your dreams",
      heroInspire: "Renovation\nthat inspires",
      heroComfort: "Creating comfort in every meter",
      cta: "Calculate Budget",
      readySolutions: "READY SOLUTIONS",
      popularServices: "Popular Services",
      allServices: "All Services",
      fromPrice: "from $100",
      viewAll: "View All",
      advantages: "OUR ADVANTAGES",
      whyUs: "Why choose us?",
      deadlineTitle: "Contract deadlines",
      deadlineDesc: "We guarantee completion on time or a penalty. Each day of delay = 1% of the cost returned.",
      transparentTitle: "Transparent Process",
      transparentDesc: "Daily photo reports in Telegram and full transparency of all work",
      materialsTitle: "Premium materials",
      materialsDesc: "We work with leading brands at wholesale prices",
      projectsCount: "Projects",
      warranty: "Warranty",
      warranty_years: "3 years",
      experience: "Experience",
      experience_years: "8 years",
      portfolioLabel: "PORTFOLIO",
      all: "All",
      stories: {
        process: "Process",
        reviews: "Reviews",
        team: "Team",
        promo: "Promo",
        details: "Details",
        now: "now",
        m: "m",
        h: "h",
        d: "d"
      },
      heroSubtitle: "Premium quality. Delivered on time by contract.",
      videoTitle: "Video Reviews",
      recentTitle: "Recent Works",
      fab: "Ask a Question",
    },
    calc: {
      title: "Smart Calculator 2.0",
      step1: "Property & Rooms",
      step2: "Apartment Area",
      step3: "Current Condition",
      step4: "Select a Package",
      types: {
        new: "New Building",
        secondary: "Secondary Housing",
        house: "House",
      },
      rooms: {
        studio: "Studio",
        1: "1 room",
        2: "2 rooms",
        3: "3 rooms",
        4: "4+ rooms",
      },
      conditions: {
        rough: "Rough Finish",
        old: "Old Renovation (Demolition)",
        white: "White Box",
      },
      packages: {
        economy: "Economy",
        economy_desc: "Wallpaper, linoleum, stretch ceiling",
        comfort: "Comfort",
        comfort_desc: "Paint, laminate, drywall",
        premium: "Premium",
        premium_desc: "Decorative plaster, parquet, complex ceilings",
      },
      result: {
        title: "Preliminary Estimate",
        work: "Labor Cost",
        rough_mat: "Rough Materials",
        finish_mat: "Finishing Materials",
        total: "Turnkey Total:",
        range: "$",
        download: "Download Estimate (PDF)",
        cta: "Call an Engineer",
      },
      next: "Next",
      back: "Back",
      subtitle: "CALCULATOR",
      est_cost: "Estimated Cost",
      area_label: "Apartment Area",
      sqm: "sqm",
      type_label: "Room Type",
      level_label: "Renovation Level",
      cta_main: "Order Calculation",
      disclaimer: "The calculation is preliminary. The exact cost is determined after measurement and a detailed estimate.",
      modal: {
        title: "Leave your contact",
        name: "Your name",
        name_placeholder: "Enter your name",
        phone: "Phone",
        phone_placeholder: "+998 90 354 76 66",
        summary: "Calculation",
        area: "Area",
        type: "Type",
        level: "Level",
        total: "Estimated Cost",
        submit: "Send Request",
        manager_note: "A manager will contact you within 15 minutes",
        success_title: "Request Sent!",
        success_text: "Our manager will contact you soon for details",
        success_btn: "Great"
      }
    },
    portfolio: {
      title: "Project Gallery",
      filters: {
        all: "All",
        kitchen: "Kitchen",
        living: "Living Room",
        bath: "Bathroom",
        bedroom: "Bedroom",
        full: "Full Project",
      },
      cta: "I want this too",
      location: "Location",
      area: "Area",
      term: "Duration",
      portfolio_upper: "PORTFOLIO",
      all_projects: "All projects",
      new_buildings: "New buildings",
      turnkey_renovation: "Full turnkey renovation",
      project_not_found: "Project not found",
    },
    project: {
      share: "Share",
      term_badge: "Term: 3.5 months",
      location: "Location",
      area: "Area",
      cost: "Labor Cost",
      term: "Implementation Period",
      what_was_done: "What was done",
      read_more: "Read full text",
      hide: "Hide",
      materials: "Materials Used",
      cta: "I want the exact same renovation",
      video_review: "Project Video Review",
      team_title: "Project Team",
      works_title: "Completed Works",
      about_title: "About Project",
      stats: {
        budget: "Budget",
        term: "Term",
        area: "Area",
        type: "Type"
      }
    },
    services: {
      price_list: "PRICE LIST",
      catalog_title: "Works catalog",
      catalog_desc: "Transparent pricing for all types of renovation works",
      electrical: "Electrical",
      finishing: "Finishing",
      tiles: "Tiles",
      plumbing: "Plumbing",
      full_list: "Full price list",
      need_consultation: "Need consultation?",
      consultation_desc: "We will help you calculate the exact cost of renovation",
      cta: "Calculate budget",
    },
    dashboard: {
      title: "My Property",
      profile: "Profile",
      greeting: "Hello, Alisher!",
      address: "15 Nukus St., Apt 42",
      manager: "Your manager",
      call: "Call",
      contract: "Contract #",
      id: "Your ID:",
      loading: "Loading...",
      project: "Project",
      events: "Events",
      admin_panel: "Admin Panel",
      admin_desc: "Project Management",
      my_project_title: "My Project",
      status: {
        process: "In Progress",
        finished: "Finished",
        new: "New"
      },
      stage: "Stage:",
      paid: "Paid:",
      payment_progress: "Payment progress",
      no_projects: "No active projects",
      no_projects_desc: "Start your perfect renovation with us. Calculate the cost in 1 minute.",
      calc_btn: "Calculate Renovation Cost",
      contact_manager: "Contact manager",
      finance: {
        title: "Finance",
        total: "Total Estimate",
        paid: "Paid",
        left: "Balance Due",
        history: "Payment History",
        transactions: "transactions",
      },
      stage_info: {
        title: "Current Status",
        name: "Stage 3: Fine Finishing",
        status: "In Progress",
        forecast: "Forecasted Completion:",
        updates: "updates",
      },
      timeline: {
        title: "Work Timeline",
        today: "Today",
        yesterday: "Yesterday",
        comment1: "Finished tiling in the kitchen",
        comment2: "Materials delivered",
      },
      docs: {
        title: "Documents",
        contract: "Contract #145",
        estimate: "Estimate (Version 2)",
        act: "Rough Works Acceptance Certificate",
      },
      auth: {
        placeholder: "Contract Number (e.g., 1234)",
        btn: "Log into Cabinet",
      },
      video: {
        btn: "Watch 24/7 Broadcast",
        offline: "Camera temporarily unavailable",
      },
    },
    booking: {
      title: "Sign up for Measurement",
      share_contact: "Share Telegram Contact",
      or: "Or enter manually",
      name: "Your Name",
      name_placeholder: "Alisher",
      phone: "Phone Number",
      date: "Convenient Time",
      address: "Property Address",
      comment: "Wishes",
      submit: "Confirm Meeting",
      success: {
        title: "Request Accepted!",
        text: "An engineer will contact you to confirm the time.",
        home: "To Homepage",
      },
    },
    admin: {
      nav: {
        dashboard: "Dashboard",
        crm: "Leads (CRM)",
        users: "Users",
        projects: "Projects",
        portfolio: "Portfolio",
        services: "Services",
        stories: "Stories",
        catalog: "Shop",
        settings: "Settings",
        logout: "Logout",
      },
      dashboard: {
        new_leads: "New Leads",
        active_projects: "Active Projects",
        total_money: "Total Turnover",
        recent_activity: "Recent Activity",
        completed_projects: "Completed Projects",
        total_clients: "Total Clients",
        waiting_measuring: "Waiting Measurement",
        average_rating: "Average Rating",
        upcoming_tasks: "Upcoming Tasks",
        overview: "Overview",
        in_progress_now: "in progress now",
        require_processing: "require processing",
        based_on_projects: "sum of all payments",
        about: "About",
        total: "Total",
        recent_time: "Recently",
        system: "System",
        new_lead: "New Lead:",
        unknown: "Unknown",
        updated: "Updated",
        project: "Project",
        measuring: "Measurement:",
        call: "Call",
        periods: {
          today: 'Today',
          week: 'This Week',
          month: 'This Month',
          all: 'All Time'
        },
        filter: "Filter",
        all_tasks: "All Tasks",
        history: "History"
      },
      crm: {
        title: "Lead Management",
        status: {
          new: "New",
          process: "In Progress",
          measure: "Measurement",
          contract: "Contract",
          reject: "Rejected",
          contacted: "Contacted",
          all: "All"
        },
        sources: {
          calculator: "Calculator",
          booking: "Measurement Booking",
          phone: "Phone",
          other: "Other"
        },
        stats: {
          total: "Total Leads",
          new: "New",
          in_progress: "In Progress",
          contracts: "Contracts"
        },
        search_placeholder: "Search by name or phone...",
        no_leads_title: "No leads yet",
        no_leads_desc: "New leads will appear here when clients fill the form",
        no_name: "Name not specified",
        fields: {
          area: "Area",
          type: "Type",
          budget: "Budget"
        },
        actions: {
          call: "Call",
          accept: "Accept"
        },
        convert: "Create Project",
      },
      project: {
        title: "Project Management",
        finance_title: "Finance",
        total_estimate: "Total Estimate",
        paid_amount: "Paid",
        timeline_title: "Event Timeline",
        add_event: "Add Event",
        event_placeholder: "What was done? (e.g., Laminate delivered)",
        upload_photo: "Upload Photo",
        upload_doc: "Upload Document (PDF)",
        add_project: "Add Project",
        search_placeholder: "Search by RC name, client or address...",
        status: {
          in_progress: "In Progress",
          finished: "Finished",
          new: "New"
        },
        payment: "Payment",
        paid: "Paid:",
        new_project: "New Project",
        create_desc: "Create a new object card for tracking",
        input_lang: "Input Language",
        client: "Client",
        select_from_db: "Select from database",
        search_client: "Search client...",
        or_manual: "Or manually",
        fio: "Full Name",
        phone: "Phone",
        object: "Object",
        address: "Address",
        contract_details: "Contract number / Estimate",
        total: "Total",
        tg_id: "Telegram ID",
        terms: "Timeline",
        start_date: "Start date",
        deadline: "Deadline (Forecast)",
        cancel: "Cancel",
        create: "Create Project",
        foreman_salary: "Foreman Salary",
        monthly_rate: "Monthly rate",
        add_month: "Add month",
        paid_label: "Paid",
        not_paid_label: "Pending",
        video_review: "Video Review",
        upload_video: "Upload Video",
        video_url: "Video URL",
        works_completed: "Completed Works",
        team: "Project Team",
        budget: "Budget",
        currency: "sum",
        edit_project: "Edit Project",
        general_tab: "General",
        details_tab: "Details",
        media_tab: "Media",
        info: "Information",
        project_name: "Project / Building Name",
        room_type: "Room Type",
        area_sqm: "Area (sqm)",
        term_realization: "Term of Realization",
        details: "Details",
        description: "Description",
        tags: "Tags (comma separated)",
        materials_input: "Materials (comma separated)",
        new_building: "New Building",
        new_building_desc: "Check if this is a new residential complex",
        works_placeholder: "Rough: Screed, Plaster\nFinish: Paint, Wallpaper",
        team_placeholder: "John Doe | Designer\nJane Smith | Foreman",
        photo_before: "Photo BEFORE",
        photo_after: "Photo AFTER",
        gallery: "Project Gallery (All photos)",
        save_changes: "Save Changes",
        publish: "Publish Project",
        location: "Location",
      },
      catalog: {
        title: "Product Catalog",
        add_product: "Add Product",
        search: "Search by title...",
        edit: "Edit Product",
        new: "New Product",
        main_info: "Main Information",
        specs: "Specifications",
        main_photo: "Main Photo",
        gallery: "Gallery",
        video: "Video (optional)",
        upload_video: "Upload video file",
        video_url: "Video URL",
        save: "Save",
        add: "Add",
        cancel: "Cancel",
        confirm_delete: "Are you sure you want to delete this item?",
        input_lang: "Input Language",
        price_label: "Price",
        media_assets: "Media Assets",
        title_label: "Title",
        desc_label: "Description",
        category_label: "Category",
        price_uzs: "Price (UZS)"
      },
      portfolio: {
        title: "Our Works",
        add_project: "Add Project"
      },
      stories: {
        title: "Stories",
        add_story: "Add Story",
        edit_story: "Edit Story",
        new_story: "New Story",
        titles: "Titles",
        category: "Category",
        cover: "Cover",
        video_optional: "Video (optional)",
        video_review: "Video Review",
        upload_video: "Upload Video",
        video_url: "Video URL",
        save: "Save",
        add: "Add",
        confirm_delete: "Are you sure?",
        required_fields: "Fill in all required fields"
      },
      users: {
        title: "Bot Users",
        search: "Search by name, username or Telegram ID...",
        table: {
          user: "User",
          tg_id: "Telegram ID",
          status: "Status",
          actions: "Actions"
        },
        status: {
          active: "Active",
          inactive: "Inactive"
        },
        not_found: "No users found"
      },
      settings: {
        title: "Prices & Calculator",
        base_price: "Base price per m²",
      }
    }
  }
};
